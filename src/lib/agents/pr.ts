...
export async function runPr(
  ...
  const title = `[${APP_NAME}] ${ctx.request.title}`;
    ...
    try {
      ...
      if (build.committed) {
        ...
        opened = await openPrWithFile({
          ...
          repo: ctx.repo,
          branch: build.branch,
          filePath: `docs/${APP_NAME.toLowerCase()}/${ctx.request.id}.md`,
          fileContents: brdDoc(ctx, brd),
          commitMessage: `${APP_NAME} requirements: ${ctx.request.title}`,
          prTitle: title,
...
_Generated by ${APP_NAME} from an approved stakeholder request._
...
function prBody(ctx: AgentContext, brd: BrdResult, build: BuildResult): string {
  const footer = build.committed
    ? `### Changes\n${build.filesChanged.map((f) => `- \`${f}\``).join("\n")}\n\n${build.summary}\n\n> Auto-implemented by ${APP_NAME}'s builder agent from the approved requirements. Review before merging.`
    : `> This PR carries the approved requirements. Connect a repo + API key so the builder agent can implement them as real code.`;

  return `Opened by **${APP_NAME}** from an approved stakeholder request.
